...
# 14 book
# 10 cover
#mostCommonWords=$(cat * | tr -c '[:alnum:]' '[\n*]' | sort | uniq -c | sort -nr | head  -10 )
mostCommonWords=$(cat * | tr -d '[:punct:]' | sed -e 's/\s/\n/g'| tr '[A-Z]' '[a-z]'  | sort | uniq  -c | sort -nr | head -10)
...
fixedBook=$(cat * | perl -p -e 's/(!?")i/"I/g' | perl -p -e 's/(!?\s)i(!?\s)/ I /g')
