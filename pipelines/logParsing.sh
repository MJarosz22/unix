...
accessData=$(grep -F 'POST' access_log | awk 'BEGIN{OFS=","} {print $4,$1,$7,$9, $10}' | cut -c 2-)
...
largestResponse=$(grep -F 'POST' access_log | awk 'BEGIN{OFS=","} {print $10, $1, $7}'|sort -n -r|cut -d"," -f2-3|head -1)
...
# Hint: you could re-use the `accessData` variable to make it easier.
mostRequests=$(grep -F 'POST' access_log | awk 'BEGIN{OFS=","} {print $1}'|sort|uniq -c|sort -r|head -1)
